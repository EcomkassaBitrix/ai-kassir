...
// чат сам покажет ему простой выбор магазина, при следующих визитах уже не спросит.`
      ...
      {
        title: 'Внешний вид окна — адаптируется под экран автоматически',
        description:
          'На мобильных экранах (по умолчанию ≤640px ширины) окно чата само раскрывается на весь экран — так удобнее печатать и читать переписку на телефоне. На десктопе остаётся компактный попап с затемнением фона. Переключение работает и «на лету»: если посетитель повернёт телефон или изменит размер окна браузера, чат подстроится сам. При необходимости поведение и размеры можно переопределить — все параметры необязательны.',
        example: `AiCashier.init({
  token: ecomkassaApiToken,

  width: 440,               // ширина попапа на десктопе, px (по умолчанию 440)
  height: 680,               // высота попапа на десктопе, px (по умолчанию 680)
  mobileBreakpoint: 640,     // ниже этой ширины экрана — окно раскрывается на весь экран
  fullscreenOnMobile: true,  // false — никогда не раскрывать на весь экран
  fullscreen: false          // true — всегда весь экран, даже на десктопе
});`
      },
      {
