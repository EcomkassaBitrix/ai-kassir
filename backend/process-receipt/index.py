...
def get_ai_completion(user_text: str, settings: dict, context: str = '') -> Optional[Dict[str, Any]]:
    ...
    prompt = f"""ИИ кассир. Преобразуй запрос в JSON для Ecomkassa API.

Запрос: "{user_text}"{context_part}

Голосовые запросы: исправляй ошибки ("150₽"="150 рублей"="150р", "иван собака мейл"="ivan@mail").

name (товар/услуга):
- Конкретное название: "кофе", "стрижка", "консультация психолога"
- НЕ используй: "товар", "услуга", "чек", "создай", "ссылка"
- Если только цена без названия → error: "Что продаёшь за X₽?"

Обязательные поля:
- price: ОБЯЗАТЕЛЬНА (если нет → error)
- name: ОБЯЗАТЕЛЬНО (если нет → error)
- email/phone: необязательны (null = бэкэнд подставит дефолтный)

payment_object:
- service: психолог, стрижка, массаж, консультация, ремонт, доставка, урок
- commodity: кофе, телефон, мебель, одежда, продукты

payment_method: full_payment (по умолчанию), partial_payment (взнос+кредит), credit (только кредит)

payments:
- type: "0"=наличные, "1"=безнал, "2"=предоплата, "3"=кредит
- Сумма payments = сумме товаров
- Примеры: "500 наличными остальное картой" → [{{"type":"0","sum":500}},{{"type":"1","sum":остаток}}]

Формат ответа:
{{"operation_type":"sell","items":[{{"name":"кофе","price":200,"quantity":1,"measure":"шт","vat":"none","payment_method":"full_payment","payment_object":"commodity"}}],"client":{{"email":null,"phone":null}},"payments":[{{"type":"1","sum":200}}]}}

Если не хватает данных:
{{"error":"Укажи название и цену. Пример: кофе 200₽"}}
            ...
            if qty_int > 0:
                ...
                items_for_payload.append({
                    ...
                    'payment_object': {
                        ...
                        'casino_payment': 26,
                        'agent_payment': 27,
                        'excisable_goods_without_marking': 30,
                        'excisable_goods_with_marking': 31,
                        'goods_without_marking': 32,
                        'goods_with_marking': 33
